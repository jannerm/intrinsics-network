...
cmd = torch.CmdLine()
cmd:option('-net_path', 'saved_alternate/')
cmd:option('-base_class', 'bottle')
...
cmd:option('-multipliers', '10:1,1,1', 'albedo, shading, supervised')
...
local model, fixed = alternate_composer(channels_net)
...
local const_inp, const_par, const_targ, const_mask = load_sequential( paths.concat(opt.image_path, opt.test_path), opt.array_path, {'albedo', 'shading'}, opt.test_size, opt.channels, opt.m, opt.n, opt.param_dim, true )
-- const_inp = const_inp:cuda()
-- const_par = const_par:cuda()
-- const_targ = const_targ:cuda()
local const_intrinsics = convert_for_alt(const_targ)
...
while true do 
    ...
    if epoch % opt.val_freq == 0 then
        local errors, preds, truth = trainer:validate_alternate(const_inp, const_mask, fixed, const_intrinsics, true)
        local albedo_err, shading_err, render_err = unpack(errors)
        print(string.format('\n#### Intrinsic Error     albedo:  %.8f | normals:   %.8f', albedo_err, shading_err) )
        print(string.format('                           render: %.8f\n', render_err) )
        trainer:log_intrinsics(opt.save_path, errors, {'albedo', 'shading', 'render'})
    ...
    local learn_output, fixed_output = convert_intrinsics_alternate(sup_inputs, sup_targets)
    ...
    local unsup_err, sup_err = trainer:train_alternate_interleaved(unsup_inputs, fixed, unsup_mults, sup_mult, sup_inputs, learn_output, opt.repeats, opt.batchSize)
    ...
    local formatted = trainer:visualize_alternate(val_inp, val_masks, fixed)
